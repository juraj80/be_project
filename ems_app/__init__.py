...
import os
import ems_app
import ems_app.controllers.home_controller as home
import ems_app.controllers.emps_controller as emps
import ems_app.controllers.account_controller as account
import ems_app.controllers.admin_controller as admin
from ems_app.data.dbsession import DbSessionFactory


def init_db(config):
    top_folder = os.path.dirname(ems_app.__file__)
    rel_folder = os.path.join('db', 'ems_app.sqlite')
...
def init_routing(config):
    ...
    add_controller_routes(config, emps.EmpsController, 'emps')
